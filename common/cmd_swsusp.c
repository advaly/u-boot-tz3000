...
#ifdef FAST_SWSUSP
/*
 * Special function to support fast swsusp by reading a group of several
 * 4k pages at once
 */
static int page_readpages(u32 page, void **pages, u32 num)
{
	__u32 cnt;
	int blk_per_page;

	blk_per_page = PAGE_SIZE / swap_dev->blksz;
	cnt = swap_dev->block_readpages(swap_dev->dev,
					swap_info.start + page * blk_per_page,
					blk_per_page * num,
					pages);
	return cnt != blk_per_page * num;
}
#endif /* FAST_SWSUSP */
...
static void swsusp_finish (void *userdata)
{
	...
	for (;;) {
		...
		if (!remap_orig || (unsigned long)remap_orig[idx] == ~0UL)
			break;
...
{
	meta_map = memalign(ARCH_DMA_MINALIGN, PAGE_SIZE);
	if (!meta_map)
		return -1;
	meta_map_next = 0;
	meta_map_curr = 0;
...
static int raw_page_get_next_entry(void)
{
	...
	return meta_map->entries[meta_idx++];
}
static int raw_page_get_next(void *buffer)
{
	int page = raw_page_get_next_entry();
	if (page <= 0)
		return page;
	if (page_read(page, buffer))
		return -1;
...
int image_page_init (int compressed)
{
	if (compressed) {
		...
		cmp_buf = memalign(ARCH_DMA_MINALIGN, LZO_CMP_SIZE);
		if (!unc_buf || !cmp_buf)
			return -1;
	}
...
int do_swsusp (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	char *stack_addr;
#ifdef FAST_SWSUSP
	int do_fast_swsusp = 1;
	void **page_addrs = NULL;
	u32 page_start_entry = 0;
	int num_page_addr = 0;
#endif /* FAST_SWSUSP */
	...
	spare_page = memalign(ARCH_DMA_MINALIGN, PAGE_SIZE);
	if (!spare_page)
		goto mem_err;
	...
	if (strcmp(strchr(argv[0], '.') ?: "", ".ssboot") != 0 &&
	    memcmp(swsusp_header->sig, swsusp_header->orig_sig, 10)) {
...
#ifdef FAST_SWSUSP
	if (image_compressed ||
	    !swap_dev->block_readpages ||
	    strcmp(getenv("fastswsusp") ?: "", "no") == 0)
		do_fast_swsusp = 0;
	if (do_fast_swsusp) {
		page_addrs = malloc(ARRAY_SIZE(meta_map->entries) *
				    sizeof(*page_addrs));
		if (!page_addrs)
			goto mem_err;
	}
	...
	for (i = 0; i < nr_pfn_pages; i++) {
		u32 idx;
		pfn_pages[i] = memalign(ARCH_DMA_MINALIGN, PAGE_SIZE);
		if (!pfn_pages[i])
			goto mem_err;
...
#ifdef FAST_SWSUSP
		if (do_fast_swsusp) {
			int entry = raw_page_get_next_entry();
			if (entry <= 0)
				goto read_err;
			if (i == 0) {
				num_page_addr = 0;
				page_start_entry = entry;
			} else if (page_start_entry + num_page_addr != entry ||
				   num_page_addr >=
				   ARRAY_SIZE(meta_map->entries)) {
				/* non-continuoues page found */
				if (page_readpages(page_start_entry,
						   page_addrs, num_page_addr))
					goto read_err;
				num_page_addr = 0;
				page_start_entry = entry;
			}
			page_addrs[num_page_addr] = pg2addr(page);
			num_page_addr++;
			if (i == swsusp_info->image_pages - 1) {
				/* read last continuous pages */
				if (page_readpages(page_start_entry,
						   page_addrs, num_page_addr))
					goto read_err;
			}
...
U_BOOT_CMD(swsusp, 4, 0, do_swsusp,
	...
	"[.ssboot] <interface> [<dev[:part]>] [<offset>]"
);
